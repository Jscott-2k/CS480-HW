...
import './CobSoulo.css';
import { Menu, Image, Button, Icon, Label, MenuItemProps, Segment, MenuItem, Dropdown, Input, MenuHeader, Message, Container, Divider, Form, FormInput, FormField, InputOnChangeData, Modal } from 'semantic-ui-react'

import { Navigate } from "react-router-dom"
import { debug } from 'console';
import CobSouloGameDark from './CobSouloGameDark';
import CobSouloLoginBonusDark from './CobSouloLoginBonusDark';
import CobSouloStoreDark from './CobSouloStoreDark';
...
class CobSouloDark extends Component<Props, States> {

    constructor(props: Props) {
        super(props);
        this.state = {
            username: "", password: "", login: false, day: 0, page: 0, gold: 0, silver: 0, brass: 0,termsOpen:false,termsRead:false
        }
        ...
        if (this.state.login) {
            ...
            if (this.state.page == 0) {
                return <UserContextProvider day={this.state.day} gold={this.state.gold} silver={this.state.silver} brass={this.state.brass}>
                    ...
                    ></CobSouloGameDark></UserContextProvider>
            } else if (this.state.page == 1) {
                return <UserContextProvider day={this.state.day} gold={this.state.gold} silver={this.state.silver} brass={this.state.brass}> <CobSouloLoginBonusDark day={this.state.day}
                    onClaimLoginGame={(nextDay) => { this.setState({ page: 0, day: nextDay }) }} onCancelClaim={() => { this.setState({ page: 0, day: this.state.day }) }}>
                </CobSouloLoginBonusDark></UserContextProvider>
            } else if (this.state.page == 2) {
                return <UserContextProvider day={this.state.day} gold={this.state.gold} silver={this.state.silver} brass={this.state.brass}> <CobSouloStoreDark recharge={false} day={this.state.day}
                    onPurchaseItem={(item) => { this.setState({ page: 0 }) }} onReturn={() => { this.setState({ page: 0 }) }}>
                </CobSouloStoreDark></UserContextProvider>
            } else if (this.state.page == 3) {
                return <UserContextProvider day={this.state.day} gold={this.state.gold} silver={this.state.silver} brass={this.state.brass}> <CobSouloStoreDark recharge={true} day={this.state.day}
                    onPurchaseItem={(item) => { this.setState({ page: 0 }) }} onReturn={() => { this.setState({ page: 0 }) }}>
                </CobSouloStoreDark></UserContextProvider>
            }
        }
                    ...
                    <div className='HW3PentagramCover'>
                        ...
                        <div >
                            ...
                            <div className="HW3WelcomeLoginInputs">
                                ...
                                <div>
                                    <Button inverted color="red" type='submit' size='huge' onClick={this.handleSubmit}>Login</Button></div>

                            </div>
                        </div>
                        <div className='HW3Disclaimer'>
